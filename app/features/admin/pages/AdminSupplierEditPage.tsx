...
export default function AdminSupplierEditPage() {
  const navigate = useNavigate();
  const revalidator = useRevalidator();
  const params = useParams();
  const location = useLocation() as { state?: { supplier?: Supplier } };
  const supplierFromState = location.state?.supplier;
  const [isLoading, setIsLoading] = useState(false);
  const [form, setForm] = useState<Partial<Supplier>>({
    name: supplierFromState?.name || '',
    email: supplierFromState?.email || '',
    phone: supplierFromState?.phone || '',
    address: supplierFromState?.address || '',
    isVerified: supplierFromState?.isVerified ?? false,
    isActive: supplierFromState?.isActive ?? true,
  });
